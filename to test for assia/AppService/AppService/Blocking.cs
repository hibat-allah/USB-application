...
namespace AppService
{
    public class Blocking
    {
        ...
        public static void DeleteAllRegistryValues(string registryPath)
        {
            try
            {
                Console.WriteLine($"Attempting to delete all values under registry path: {registryPath}");
                ...
                using (RegistryKey key = Registry.LocalMachine.OpenSubKey(registryPath, true))
                {
                    ...
                    if (key != null)
                    {
                        ...
                        foreach (string valueName in valueNames)
                        {
                            ...
                            {
                                Console.WriteLine($"Error deleting value '{valueName}': {ex.Message}");
                            }
                        }
                    }
                    else
            ...
            {
                Console.WriteLine($"An error occurred while deleting values from registry path '{registryPath}': {ex.Message}");
            }
        }
        //------------------- Activating the Registries ------------------------
        public static void ActivateStartValue(string registryPath,int value)
        {
            try
            {
                ...
                using (RegistryKey key = Registry.LocalMachine.OpenSubKey(registryPath, true))
                {
                    ...
                    if (key != null)
                    {
                        // Modifier la valeur de Start à 3 si il est autorisé sinon a 4
